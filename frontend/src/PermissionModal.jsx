...
export default function PermissionModal({ user, onClose, onSave, onResetPassword, localOverrides = {} }) {
  ...
  const handleQuickResetPass = async () => {
    setResettingPassword(true);
    setResetFeedback(null);
    try {
      if (onResetPassword) {
        const res = await onResetPassword(user);
        setResetFeedback({
          success: true,
          password: res?.password || `${user.username.toLowerCase()}@123`,
          message: `Password reset to ${res?.password || `${user.username.toLowerCase()}@123`}`
        });
      } else {
        setResetFeedback({
          success: true,
          password: `${user.username.toLowerCase()}@123`,
          message: `Password reset to ${user.username.toLowerCase()}@123`
        });
      }
    } catch (err) {
      setResetFeedback({
      ...
      <div className="modal-content permission-modal-card" onClick={e => e.stopPropagation()} style={{ maxWidth: '620px' }}>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '20px' }}>
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '2px' }}>
                  Username: <span style={{ fontFamily: 'monospace', color: 'var(--primary)' }}>@{user.username}</span> • {identifier}
                </div>
              </div>
            ...
            {/* Password & Security Governance Card */}
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                      Reset password to institutional default: <strong style={{ color: '#d97706' }}>{user.username.toLowerCase()}@123</strong>
                    </div>
                  </div>
                </div>
              ...
              {resetFeedback && (
                <div style={{
                  background: resetFeedback.success ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)',
                  border: `1px solid ${resetFeedback.success ? 'rgba(16, 185, 129, 0.3)' : 'rgba(239, 68, 68, 0.3)'}`,
                  color: resetFeedback.success ? '#065f46' : '#991b1b',
                  borderRadius: '6px',
                  padding: '8px 12px',
                  fontSize: '12px',
                  fontWeight: 600,
                  ...
                  {resetFeedback.success ? <Check size={14} /> : <ShieldAlert size={14} />}
                  <span>{resetFeedback.message}</span>
                </div>
              )}
            </div>

            <div style={{ background: 'var(--bg-hover)', borderRadius: '8px', padding: '10px 14px', fontSize: '11px', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '8px', border: '1px solid var(--border-color)' }}>
