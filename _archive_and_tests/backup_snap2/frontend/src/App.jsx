...
export default function App() {
  ...
  // Teacher Leaves & Attendance Regularizations Governance (Admin Exclusive)
  ...
  const loadAllData = async () => {
    setLoading(true);
    try {
      const [stuRes, tchRes, feeRes, salRes, clsRes, tlRes, trRes, ttRes] = await Promise.all([
  ...
  // Review Teacher Leave Request (Admin Exclusive)
  const handleReviewTeacherLeave = async (e) => {
    ...
    try {
      await api.reviewTeacherLeave(selectedTeacherLeaveReview.id, {
        ...
        admin_remarks: teacherLeaveRemarks.trim(),
        reviewed_at: new Date().toISOString()
      });
      // Optimistic local update — notification badge drops instantly
      setTeacherLeaves(prev => prev.map(l =>
        l.id === selectedTeacherLeaveReview.id
          ? { ...l, status: teacherLeaveDecision, admin_remarks: teacherLeaveRemarks.trim(), reviewed_at: new Date().toISOString() }
          : l
      ));
      setSelectedTeacherLeaveReview(null);
      setTeacherLeaveRemarks('');
      setTeacherLeaveDecision('Approved');
      // Full DB reload to confirm persisted state
      await loadAllData();
      alert(`Faculty leave application has been ${teacherLeaveDecision.toUpperCase()} and saved to the database!`);
    } catch (err) {
  ...
  // Review Teacher Attendance Regularization (Admin Exclusive)
  const handleReviewTeacherReg = async (e) => {
    ...
    try {
      await api.reviewTeacherAttendanceRegularization(selectedTeacherRegReview.id, {
        ...
        admin_remarks: teacherRegRemarks.trim(),
        reviewed_at: new Date().toISOString()
      });
      // Optimistic local update — notification badge drops instantly
      setTeacherRegularizations(prev => prev.map(r =>
        r.id === selectedTeacherRegReview.id
          ? { ...r, status: teacherRegDecision, admin_remarks: teacherRegRemarks.trim(), reviewed_at: new Date().toISOString() }
          : r
      ));
      ...
      // Full DB reload to confirm persisted state
      await loadAllData();
      alert(`Faculty attendance regularization has been ${teacherRegDecision.toUpperCase()} and saved to the database!`);
    } catch (err) {
  ...
  const totalSalariesPaid = salaries.reduce((sum, s) => sum + (parseFloat(s.net_salary) || 0), 0);
  ...
  }, [teacherLeaves, teacherRegularizations, fees, salaries]);
  ...
  const handleSelectStudentForFee = (studentId) => {
    ...
    const gross = studentPrior.length > 0 ? (parseFloat(studentPrior[0].gross_amount) || 25000) : 25000;
  ...
  const studentFeeLedger = useMemo(() => {
    ...
    return students.map(student => {
      ...
      if (studentReceipts.length > 0) {
        ...
        const gross = parseFloat(latest.gross_amount) || 25000;
  ...
  const liveDeductionsSalary = useMemo(() => {
    return (parseFloat(newSalary.provident_fund) || 0) +
      (parseFloat(newSalary.tax_deducted_tds) || 0);
  }, [newSalary]);

  const liveNetSalary = useMemo(() => {
    return liveGrossSalary - liveDeductionsSalary;
  ...
  const teacherPayrollLedger = useMemo(() => {
    ...
    return teachers.map(teacher => {
      ...
      if (teacherSlips.length > 0) {
        ...
        return {
          teacher,
          payslip_no: latest.payslip_no,
          salary_month: latest.salary_month,
          salary_year: latest.salary_year,
          gross_earnings: latest.gross_earnings,
      ...
      <aside className="sidebar">
        <div className="sidebar-header">
          ...
          <div className="brand-title">
            ...
            <span className="brand-badge">C++ & React System</span>
          </div>
        </div>
        ...
        <nav className="nav-section">
          ...
          <span className="nav-label" style={{ marginTop: '16px' }}>Governance & Approvals</span>
      ...
      <main className="main-content">
        ...
        <header className="top-header">
          <div className="page-title-box">
            ...
            <p>Integrated with C++ REST Backend & Supabase Cloud PostgreSQL</p>
          </div>
                    ...
                    {activeAdminAlerts.length === 0 ? (
                      ...
                      activeAdminAlerts.map((notif) => {
                        return (
                          ...
                          >
                            <div className="notif-item-icon" style={{ background: notif.category === 'teacher-leave' ? 'rgba(56, 189, 248, 0.15)' : notif.category === 'attendance' ? 'rgba(52, 211, 153, 0.15)' : notif.category === 'salary' ? 'rgba(245, 158, 11, 0.15)' : 'rgba(168, 85, 247, 0.15)', color: notif.category === 'teacher-leave' ? '#38bdf8' : notif.category === 'attendance' ? '#34d399' : notif.category === 'salary' ? '#f59e0b' : '#c084fc' }}>
                              {notif.category === 'teacher-leave' && <CalendarCheck size={18} />}
                              {notif.category === 'attendance' && <Clock size={18} />}
                ...
                <tbody>
                  {filteredStudents.map(student => (
                    <tr key={student.id}>
                      ...
                      <td>{student.classes ? `${student.classes.class_name} - ${student.classes.section}` : 'Unassigned'}</td>
                      <td>{student.email}</td>
                      <td>{student.phone || '—'}</td>
                      <td>
                        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                  ...
                  {filteredTeachers.map(t => {
                    ...
                    return (
                      <tr key={t.id}>
                        ...
                        <td>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                            ...
                            <div style={{ fontSize: '11.5px', color: '#94a3b8' }}>
                              {t.designation || 'Faculty Member'} • <span style={{ color: '#cbd5e1' }}>{t.qualification || 'M.Sc.'}</span>
                            </div>
                        ...
                        <td>
                          <div style={{ fontSize: '12.5px', color: '#e2e8f0' }}>{t.email}</div>
                          <div style={{ fontSize: '11.5px', color: '#94a3b8' }}>{t.phone || '—'}</div>
                        </td>
                        <td style={{ textAlign: 'right', paddingRight: '20px' }}>
                          <div style={{ display: 'inline-flex', gap: '6px', alignItems: 'center' }}>
                            ...
                            <button 
                              ...
                              style={{ background: 'rgba(59, 130, 246, 0.15)', color: '#60a5fa', border: '1px solid rgba(59, 130, 246, 0.3)', borderRadius: '6px', padding: '6px 8px', cursor: 'pointer' }}
                            >
                              <Pencil size={14} />
                            </button>
                            <button 
                              ...
                              style={{ background: 'rgba(245, 158, 11, 0.15)', color: '#fbbf24', border: '1px solid rgba(245, 158, 11, 0.3)', borderRadius: '6px', padding: '6px 8px', cursor: 'pointer' }}
                            >
                              <Key size={14} />
                            </button>
                            <button 
                    ...
                    {paginatedFeeRows.map(item => {
                      ...
                      return (
                        <tr key={s.id}>
                          ...
                          <td>
                            <div style={{ fontWeight: '600', color: 'white' }}>{s.first_name} {s.last_name}</div>
                            <div style={{ fontSize: '11px', color: 'var(--text-dim)' }}>{s.email || '—'}</div>
                          </td>
                          <td>
                            <span style={{ fontSize: '12px', color: '#94a3b8' }}>{classNameStr}</span>
                          </td>
                          <td>
                            {item.hasPaidRecord ? (
                              <strong style={{ color: '#38bdf8', fontSize: '12px' }}>{item.receipt_no}</strong>
                            ) : (
                              <span style={{ color: 'var(--text-dim)', fontSize: '12px' }}>— (Unpaid)</span>
                            )}
                          </td>
                          <td>₹ {item.gross_amount.toLocaleString()}</td>
                          <td>
                            <strong style={{ color: item.amount_paid > 0 ? 'var(--emerald)' : 'var(--text-dim)' }}>
                              ₹ {item.amount_paid.toLocaleString()}
                            </strong>
                          </td>
                          <td>
                            <span style={{ color: item.balance_due > 0 ? '#f87171' : 'var(--text-dim)', fontWeight: item.balance_due > 0 ? '600' : 'normal' }}>
                    ...
                    <div className="modal-body">
                      <div className="printable-slip">
                        <div className="slip-header">
                          ...
                          <p style={{ fontSize: '13px', color: '#64748b' }}>Affiliated to CBSE / State Board • Academic Session 2026-2027</p>
                          <h4 style={{ marginTop: '10px', color: '#1e3a8a', letterSpacing: '0.5px' }}>
                            STUDENT FEE PAYMENT RECEIPT & INSTALLMENT LEDGER
                          </h4>
                        </div>
                        ...
                        <div className="slip-meta-grid">
                          ...
                          <div><strong>Receipt Date:</strong> {selectedReceipt.payment_date ? new Date(selectedReceipt.payment_date).toLocaleDateString() : 'Today'}</div>
                          ...
                          <div><strong>Roll Number:</strong> {selectedReceiptStudent?.roll_number || selectedReceipt.students?.roll_number || 'N/A'}</div>
                          ...
                          <div><strong>Payment Method:</strong> {selectedReceipt.payment_method || 'Online'}</div>
                        ...
                        <div style={{ marginTop: '16px', marginBottom: '8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <h5 style={{ margin: 0, fontSize: '13px', textTransform: 'uppercase', color: '#1e3a8a', letterSpacing: '0.5px', fontWeight: 700 }}>
                            Installment Payment Breakdown & Ledger ({selectedReceiptInstallments.length} Entries)
                          </h5>
                          <span style={{ fontSize: '11px', color: '#64748b' }}>
                            All entries stored for this student
                          </span>
                        </div>
                        ...
                        <div className="slip-total-box">
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px' }}>
                            ...
                            <div style={{ fontSize: '12px', textAlign: 'right', marginTop: '6px' }}>
                              <div>Authorized By: <strong>Finance Accounts Desk</strong></div>
                              <div style={{ fontSize: '10px', color: '#64748b' }}>Greenwood Academy Automated Slip</div>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    {paginatedSalaryRows.map(item => {
                      ...
                      return (
                        <tr key={t.id}>
                          ...
                          <td>
                            <strong style={{ fontFamily: 'monospace', fontSize: '12px', color: '#93c5fd' }}>
                              {item.payslip_no}
                            </strong>
                          </td>
                          <td>₹ {parseFloat(item.gross_earnings).toLocaleString()}</td>
                          <td style={{ color: '#f87171' }}>₹ {parseFloat(item.total_deductions).toLocaleString()}</td>
                          <td>
              ...
              {selectedPaySlip && (
                <div className="modal-overlay" onClick={() => setSelectedPaySlip(null)}>
                  <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '820px' }}>
                    <div className="modal-header">
                      <div>
                        <h2>Official Faculty Salary Pay-Slip</h2>
                        <p style={{ fontSize: '12px', color: 'var(--text-muted)', margin: '2px 0 0 0' }}>
                          Greenwood International Academy • Monthly Payroll & Historical Disbursement Slip
                        </p>
                      </div>
                      <button className="btn-secondary" onClick={() => window.print()}>
                    ...
                    <div className="modal-body">
                      <div className="printable-slip">
                        <div className="slip-header">
                          <h2>GREENWOOD INTERNATIONAL ACADEMY</h2>
                          <p style={{ fontSize: '13px', color: '#64748b' }}>Affiliated to CBSE / State Board • Human Resources & Payroll Division</p>
                          <h4 style={{ marginTop: '10px', color: '#065f46', letterSpacing: '0.5px' }}>
                            OFFICIAL SALARY DISBURSEMENT PAY-SLIP & HISTORY LEDGER
                          </h4>
                        </div>

                        <div className="slip-meta-grid">
                          <div><strong>Pay-Slip No:</strong> <span style={{ fontFamily: 'monospace', fontWeight: 600 }}>{selectedPaySlip.payslip_no}</span></div>
                          <div><strong>Disbursement Date:</strong> {selectedPaySlip.payment_date ? new Date(selectedPaySlip.payment_date).toLocaleDateString() : 'Today'}</div>
                          <div><strong>Faculty Name:</strong> {selectedPaySlipTeacher?.first_name || selectedPaySlip.teachers?.first_name} {selectedPaySlipTeacher?.last_name || selectedPaySlip.teachers?.last_name}</div>
                          <div><strong>Employee ID:</strong> {selectedPaySlipTeacher?.employee_id || selectedPaySlip.teachers?.employee_id || 'TCH-001'}</div>
                          <div><strong>Designation / Dept:</strong> {selectedPaySlipTeacher?.qualification || selectedPaySlip.teachers?.qualification || 'Senior Faculty'}</div>
                          <div><strong>Payroll Period:</strong> {selectedPaySlip.salary_month} {selectedPaySlip.salary_year}</div>
                          <div><strong>Payment Method:</strong> {selectedPaySlip.payment_method || 'Direct Bank Transfer'}</div>
                          <div><strong>Bank Transaction Ref:</strong> {selectedPaySlip.transaction_ref || 'NEFT-APPROVED-7788'}</div>
                        </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }}>
                          ...
                          <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '6px', border: '1px solid #e2e8f0' }}>
                            <h4 style={{ borderBottom: '2px solid #059669', paddingBottom: '4px', marginBottom: '8px', color: '#059669', fontSize: '12px', letterSpacing: '0.5px' }}>
                              EARNINGS BREAKDOWN (₹)
                            </h4>
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #e2e8f0', fontSize: '13px' }}>
                              <span>Basic Salary:</span> <strong>₹ {parseFloat(selectedPaySlip.basic_salary).toLocaleString()}</strong>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #e2e8f0', fontSize: '13px' }}>
                              <span>HRA Allowance:</span> <strong>₹ {parseFloat(selectedPaySlip.hra_allowance || 0).toLocaleString()}</strong>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #e2e8f0', fontSize: '13px' }}>
                              <span>DA & Special Allowance:</span> <strong>₹ {parseFloat((selectedPaySlip.da_allowance || 0) + (selectedPaySlip.special_bonus || 0)).toLocaleString()}</strong>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #e2e8f0', fontSize: '13px' }}>
                              <span>Medical Allowance:</span> <strong>₹ {parseFloat(selectedPaySlip.medical_allowance || 0).toLocaleString()}</strong>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', fontSize: '14px', fontWeight: 'bold' }}>
                              <span>Gross Earnings:</span> <span style={{ color: '#059669' }}>₹ {parseFloat(selectedPaySlip.gross_earnings).toLocaleString()}</span>
                            </div>
                          </div>

                          {/* Deductions */}
                          <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '6px', border: '1px solid #e2e8f0' }}>
                            <h4 style={{ borderBottom: '2px solid #dc2626', paddingBottom: '4px', marginBottom: '8px', color: '#dc2626', fontSize: '12px', letterSpacing: '0.5px' }}>
                              DEDUCTIONS BREAKDOWN (₹)
                            </h4>
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #e2e8f0', fontSize: '13px' }}>
                              <span>Provident Fund (PF):</span> <strong>₹ {parseFloat(selectedPaySlip.provident_fund || 0).toLocaleString()}</strong>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #e2e8f0', fontSize: '13px' }}>
                              <span>TDS / Income Tax:</span> <strong>₹ {parseFloat(selectedPaySlip.tax_deducted_tds || 0).toLocaleString()}</strong>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #e2e8f0', fontSize: '13px' }}>
                              <span>Leave Deductions:</span> <strong>₹ {parseFloat(selectedPaySlip.leave_deduction || 0).toLocaleString()}</strong>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', fontSize: '14px', fontWeight: 'bold' }}>
                              <span>Total Deductions:</span> <span style={{ color: '#dc2626' }}>₹ {parseFloat(selectedPaySlip.total_deductions).toLocaleString()}</span>
                            </div>
                          </div>
                        </div>

                        {/* Monthly Salary Disbursement Ledger & Payment History */}
                        <div style={{ marginTop: '16px', marginBottom: '8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <h5 style={{ margin: 0, fontSize: '13px', textTransform: 'uppercase', color: '#065f46', letterSpacing: '0.5px', fontWeight: 700 }}>
                            Faculty Salary Disbursement Ledger & Payment History ({selectedPaySlipHistory.length} Record(s))
                          </h5>
                          <span style={{ fontSize: '11px', color: '#64748b' }}>
                            All salary slips stored for this faculty member
                          </span>
                        </div>
                            ...
                            {selectedPaySlipHistory.map((slip, idx) => {
                              ...
                              return (
                                <tr key={slip.id || idx} className={isCurrent ? 'slip-current-row' : ''}>
                                  <td>
                                    <strong>{slip.salary_month} {slip.salary_year}</strong>
                                    {isCurrent && <span className="slip-current-tag">This Pay-Slip</span>}
                                  </td>
                                  <td style={{ fontFamily: 'monospace', fontSize: '12px' }}>{slip.payslip_no}</td>
                                  ...
                                  <td style={{ textAlign: 'center' }}>
                                    <span className="slip-installment-badge paid">
                        ...
                        {/* Slip Total & Settlement Box */}
                        <div className="slip-total-box" style={{ background: '#ecfdf5', borderColor: '#a7f3d0' }}>
                          <div>
                            <span style={{ fontSize: '13px', color: '#065f46', display: 'block', marginBottom: '2px' }}>
                            ...
                            <div style={{ fontSize: '12px', color: '#047857', marginTop: '4px' }}>
                              Total Salary Disbursed (All Stored Slips): <strong>₹ {paySlipYtdTotal.toLocaleString()}</strong>
                            </div>
                          </div>

                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '6px' }}>
                            <div className="slip-settled-stamp" style={{ borderColor: '#059669', color: '#059669' }}>
                              ✓ SALARY DISBURSED & CREDITED
                            </div>
                            <div style={{ textAlign: 'right', fontSize: '11px', color: '#64748b', marginTop: '6px' }}>
                              <div>Verified by: <strong>Head of Accounts & HR</strong></div>
                              <div>Greenwood Academy Official Payroll Slip</div>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      .map((l, idx) => {
                        ...
                        return (
                          <tr key={l.id || idx}>
                            <td>
                              <strong>{tName}</strong>
                              <div style={{ fontSize: '11px', color: '#94a3b8' }}>{tEmp} • {tDept}</div>
                            </td>
              ...
              <div className="modal-body form-grid">
                <div className="form-group full-width">
                  ...
                  <SearchableTeacherSelect 
                    ...
                    onSelect={id => setNewSalary({ ...newSalary, teacher_id: id })}
                  />
                </div>
        ...
        <div className="modal-overlay" onClick={() => setSelectedTeacherLeaveReview(null)}>
          <div className="modal-content review-leave-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '640px' }}>
            ...
            <form onSubmit={handleReviewTeacherLeave} className="review-leave-form" style={{ padding: '20px' }}>
              <div className="leave-summary-card" style={{ background: 'rgba(30, 41, 59, 0.5)', padding: '16px', borderRadius: '12px', marginBottom: '16px', border: '1px solid rgba(255,255,255,0.06)' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: '#64748b' }}>{selectedTeacherLeaveReview.teachers?.employee_id} • {selectedTeacherLeaveReview.teachers?.department}</div>
                  </div>
                  <div>
                    <span style={{ fontSize: '12px', color: '#94a3b8', display: 'block' }}>Leave Category:</span>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                  <div>
                    <span style={{ fontSize: '12px', color: '#94a3b8', display: 'block' }}>Period & Duration:</span>
                    <strong style={{ color: '#38bdf8' }}>{selectedTeacherLeaveReview.start_date} to {selectedTeacherLeaveReview.end_date}</strong>
                    <div style={{ fontSize: '11px', color: '#94a3b8' }}>{selectedTeacherLeaveReview.days_count || 1} day(s) duration</div>
                  </div>
                  <div>
                    <span style={{ fontSize: '12px', color: '#94a3b8', display: 'block' }}>Substitute Faculty:</span>
                    <span className="badge" style={{ background: 'rgba(255, 255, 255, 0.08)', color: '#e2e8f0' }}>
                ...
                <div>
                  <span style={{ fontSize: '12px', color: '#94a3b8', display: 'block' }}>Faculty Reason / Context:</span>
                  <p style={{ margin: '4px 0 0', color: '#cbd5e1', fontSize: '13px', fontStyle: 'italic', background: 'rgba(0,0,0,0.2)', padding: '8px 12px', borderRadius: '6px' }}>
              ...
              <div className="form-group" style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', marginBottom: '8px', fontSize: '13px', fontWeight: 600, color: '#f8fafc' }}>
              ...
              <div className="form-group" style={{ marginBottom: '20px' }}>
                ...
                <textarea 
                  ...
                  style={{ width: '100%', borderRadius: '8px', padding: '10px', background: '#0f172a', border: '1px solid #334155', color: '#f8fafc' }}
          ...
          <div className="modal-content review-leave-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '640px' }}>
            <div className="modal-header">
              <div className="modal-title-with-icon">
                <Clock size={22} color="#34d399" />
                <div>
                  <h3 style={{ margin: 0, fontSize: '18px' }}>Review Faculty Attendance Regularization</h3>
                  <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#94a3b8' }}>HR & Institutional Biometric Terminal Authority</p>
                </div>
              </div>
              ...
              <div className="leave-summary-card" style={{ background: 'rgba(30, 41, 59, 0.5)', padding: '16px', borderRadius: '12px', marginBottom: '16px', border: '1px solid rgba(255,255,255,0.06)' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: '#64748b' }}>{selectedTeacherRegReview.teachers?.employee_id} • {selectedTeacherRegReview.teachers?.department}</div>
                  </div>
                  <div>
                    <span style={{ fontSize: '12px', color: '#94a3b8', display: 'block' }}>Dispute Date:</span>
                    <strong style={{ color: '#38bdf8' }}>{selectedTeacherRegReview.attendance_date}</strong>
                  </div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                  <div>
                    <span style={{ fontSize: '12px', color: '#94a3b8', display: 'block' }}>Requested Adjustment:</span>
                  ...
                  <div>
                    <span style={{ fontSize: '12px', color: '#94a3b8', display: 'block' }}>Discrepancy Category:</span>
                    <span className="badge badge-info">{selectedTeacherRegReview.reason_category}</span>
                  </div>
                </div>

                <div>
                  <span style={{ fontSize: '12px', color: '#94a3b8', display: 'block' }}>Faculty Explanation & Witness Evidence:</span>
                  <p style={{ margin: '4px 0 0', color: '#cbd5e1', fontSize: '13px', fontStyle: 'italic', background: 'rgba(0,0,0,0.2)', padding: '8px 12px', borderRadius: '6px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <button
                    ...
                    style={{
                      padding: '10px',
                      borderRadius: '8px',
                      border: teacherRegDecision === 'Approved' ? '2px solid #10b981' : '1px solid #334155',
                      background: teacherRegDecision === 'Approved' ? 'rgba(16, 185, 129, 0.2)' : 'rgba(15, 23, 42, 0.6)',
                      color: teacherRegDecision === 'Approved' ? '#34d399' : '#94a3b8',
                      fontWeight: 700,
                      cursor: 'pointer',
                      display: 'flex',
                      alignItems: 'center',
                  ...
                  <button
                    ...
                    style={{
                      padding: '10px',
                      borderRadius: '8px',
                      border: teacherRegDecision === 'Rejected' ? '2px solid #ef4444' : '1px solid #334155',
                      background: teacherRegDecision === 'Rejected' ? 'rgba(239, 68, 68, 0.2)' : 'rgba(15, 23, 42, 0.6)',
                      color: teacherRegDecision === 'Rejected' ? '#f87171' : '#94a3b8',
                      fontWeight: 700,
                      cursor: 'pointer',
                      display: 'flex',
                      alignItems: 'center',
