  ...
  const loadTeacherData = async () => {
    ...
    try {
      const [teachersRes, salariesRes, studentsRes, remarksRes, leavesRes, regRes, feesRes, gradesRes, attRes, myLeavesRes, myRegRes, classesRes, timetableRes] = await Promise.allSettled([
        ...
        api.getTeacherAttendance(effectiveTeacherId, '2026-09'),
        api.getTeacherLeaves(effectiveTeacherId),
        api.getTeacherAttendanceRegularizations(effectiveTeacherId),
        api.getClasses(),
        api.getTimetable()
      ]);
      ...
      if (teachersRes.status === 'fulfilled') {
        ...
        me = list.find(t => 
          Number(t.id) === Number(currentUser?.teacher_id) || 
          Number(t.id) === Number(currentUser?.teachers?.id) || 
          String(t.employee_id) === String(currentUser?.employee_id || currentUser?.username || '') ||
          Number(t.id) === Number(currentUser?.id)
        ) || list[0] || null;
        setTeacherData(me);
      }

      const targetTid = me?.id ? Number(me.id) : effectiveTeacherId;
      ...
      if (salariesRes.status === 'fulfilled') {
        ...
        const mySal = allSal.filter(s => 
          ...
          basic_salary: Number(s.basic_salary || s.salary_base || me?.salary_base || 50000),
          allowances: (Number(s.hra_allowance || 0) + Number(s.da_allowance || 0) + Number(s.medical_allowance || 0) + Number(s.special_bonus || 0)) || Number(s.allowances || 0),
          deductions: Number(s.total_deductions || 0) || (Number(s.provident_fund || 0) + Number(s.tax_deducted_tds || 0)) || Number(s.deductions || 0),
          net_salary: Number(s.net_salary || 0) || (Number(s.basic_salary || me?.salary_base || 50000) + ((Number(s.hra_allowance || 0) + Number(s.da_allowance || 0) + Number(s.medical_allowance || 0) + Number(s.special_bonus || 0)) || 0) - (Number(s.total_deductions || 0) || 0)),
                  ...
                  {remarks.map((rem, idx) => (
                    <tr key={rem.id || idx}>
                      ...
                      <td style={{ maxWidth: '340px' }}>
                        <p style={{ margin: 0, fontSize: '13px', color: '#334155' }}>"{rem.remark}"</p>
                      </td>
                      <td>
