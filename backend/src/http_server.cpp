...
std::string HttpServer::handleRoute(const HttpRequest& req) {
    ...
    if (req.path.rfind("/api/remarks/", 0) == 0) {
        std::string idStr = req.path.substr(13);
        try {
            long long id = std::stoll(idStr);
            if (req.method == "DELETE") return dbClient.remove("student_progress_remarks", "id=eq." + std::to_string(id));
        } catch (...) {}
    }
    ...
    if (req.path.rfind("/api/student-grades/", 0) == 0) {
        std::string idStr = req.path.substr(20);
        try {
            long long id = std::stoll(idStr);
            if (req.method == "DELETE") return dbClient.remove("student_grades_results", "id=eq." + std::to_string(id));
        } catch (...) {}
    }
